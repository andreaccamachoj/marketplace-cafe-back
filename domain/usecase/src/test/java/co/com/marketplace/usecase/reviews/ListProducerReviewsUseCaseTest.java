...
class ListProducerReviewsUseCaseTest {

    @Mock private ReviewGateway reviewGateway;
    @Mock private ProducerProfileGateway producerProfileGateway;

    @InjectMocks
    private ListProducerReviewsUseCase useCase;

    private final UUID userId = UUID.randomUUID();
    private final UUID profileId = UUID.randomUUID();

    @Test
    void execute_returnsReviews_whenProfileFound() {
        ProducerProfile profile = ProducerProfile.builder().id(profileId).userId(userId).build();
        Review review = Review.builder().id(UUID.randomUUID()).productId(UUID.randomUUID())
        ...
        when(reviewGateway.findByProducerId(profileId, 0, 10)).thenReturn(Flux.just(review));
