...
class ActivateProductUseCaseTest {
    ...
    @Test
    void execute_activatesProduct_whenFound() {
        Product p = Product.builder().id(productId).name("Café").price(BigDecimal.TEN)
                .status(ProductStatus.inactive).soldCount(0)
                .createdAt(OffsetDateTime.now()).updatedAt(OffsetDateTime.now()).build();

        when(productGateway.findById(productId)).thenReturn(Mono.just(p));
        when(productGateway.updateStatus(productId, ProductStatus.active)).thenReturn(Mono.empty());
