...
class ArchiveProductUseCaseTest {
    ...
    @Test
    void execute_archives_whenOwnerMatches() {
        ProducerProfile profile = ProducerProfile.builder().id(profileId).userId(userId).build();
        Product product = Product.builder().id(productId).producerId(profileId).name("Café")
        ...
        when(productGateway.updateStatus(productId, ProductStatus.inactive)).thenReturn(Mono.empty());

        StepVerifier.create(useCase.execute(productId, userId))
                .verifyComplete();
    }
    ...
    void execute_throwsForbidden_whenOwnerMismatch() {
        ...
        ProducerProfile profile = ProducerProfile.builder().id(profileId).userId(userId).build();
        Product product = Product.builder().id(productId).producerId(otherProfileId).name("Café")
                .price(BigDecimal.TEN).status(ProductStatus.active).soldCount(0)
                .createdAt(OffsetDateTime.now()).updatedAt(OffsetDateTime.now()).build();
