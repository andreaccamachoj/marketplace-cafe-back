...
class ListMyProductsUseCaseTest {
    ...
    void execute_returnsProducts_whenProfileFound() {
        ...
        when(productGateway.findByProducerId(profileId, ProductStatus.active, 0, 10)).thenReturn(Flux.just(p));

        StepVerifier.create(useCase.execute(userId, ProductStatus.active, 0, 10))
