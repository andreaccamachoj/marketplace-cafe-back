...
class LogInventoryMovementUseCaseTest {

    @Mock private InventoryGateway inventoryGateway;

    @InjectMocks
    private LogInventoryMovementUseCase useCase;

    private final UUID productId = UUID.randomUUID();

    @Test
    void execute_adjustsInventoryWithDelta() {
        InventoryItem item = InventoryItem.builder().id(UUID.randomUUID()).productId(productId)
                .quantity(90).updatedAt(OffsetDateTime.now()).build();
        when(inventoryGateway.adjust(productId, -10)).thenReturn(Mono.just(item));

        StepVerifier.create(useCase.execute(productId, -10, "sale"))
                .expectNextMatches(i -> i.getQuantity() == 90)
                .verifyComplete();
    }
    ...
    void execute_propagatesError_whenGatewayFails() {
        ...
        StepVerifier.create(useCase.execute(productId, 5, "restock"))
