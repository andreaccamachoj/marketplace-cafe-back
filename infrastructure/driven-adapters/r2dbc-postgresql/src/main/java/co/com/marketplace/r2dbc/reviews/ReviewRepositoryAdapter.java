...
public class ReviewRepositoryAdapter implements ReviewGateway {
    ...
    public Flux<Review> findByProducerId(UUID producerId, int page, int size) {
        return db.sql("SELECT r.* FROM marketplace.reviews r " +
                ...
                .map((row, meta) -> ReviewData.builder()
                        ...
                        .build())
                .all()
                .doOnSubscribe(s -> log.debug("[ReviewRepositoryAdapter#findByProducerId] DB request: producerId={}, page={}, size={}", producerId, page, size))
